...
If you want a cleaner upload package, run the optional `prepare_deploy.js` locally (if present):

```bash
# If you have a local `prepare_deploy.js` script, run it to build a
# deployment bundle. The script is optional and may not exist in all clones.
...
CONTRIBUTOR_API_KEY=optional_contributor_key_here
